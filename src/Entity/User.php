...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    public function addThank(Thanks $thank): static
    {
        if (!$this->thanks->contains($thank)) {
            $this->thanks->add($thank);
            $thank->setFromUser($this);
        }

        return $this;
    }

    public function removeThank(Thanks $thank): static
    {
        if ($this->thanks->removeElement($thank)) {
            // set the owning side to null (unless already changed)
            if ($thank->getFromUser() === $this) {
